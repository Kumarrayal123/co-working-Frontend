// DoctorCabinRevenue.jsx - POLISHED UI matching MyRevenue (spacerevenue) style
        ...
        <svg width={220} height={220} viewBox="0 0 220 220">
          <defs>
            {slices.map((slice, index) => (
              <linearGradient key={`grad-${index}`} id={`pie-grad-dcr-${index}`} x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor={slice.gradient[0]} stopOpacity="1" />
                <stop offset="100%" stopColor={slice.gradient[1]} stopOpacity="0.9" />
              </linearGradient>
            ))}
            <filter id="glow-dcr"><feGaussianBlur stdDeviation="3" result="coloredBlur"/><feMerge><feMergeNode in="coloredBlur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          </defs>
          ...
          {slices.map((slice, index) => {
            ...
            return <path key={index} d={`M 110 110 L ${x1} ${y1} A 80 80 0 ${largeArc} 1 ${x2} ${y2} Z`} fill={`url(#pie-grad-dcr-${index})`} stroke="#fff" strokeWidth="3" filter="url(#glow-dcr)" className="transition-all duration-300 cursor-pointer hover:opacity-80 hover:scale-105" />;
          })}
      ...
      <main>
        ...
        <div className="my-revenue__header">
          <div>
            ...
            <p className="my-revenue__subtitle">Track and analyze your cabin earnings in real-time</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate("/chamberbookings")} className="my-revenue__btn my-revenue__btn--secondary">
              <Building2 size={14} />
              View All
            </button>
          </div>
        ...
        <div className="my-revenue__stats">
          ...
          <div className="my-revenue__stat">
            ...
            <div className="my-revenue__stat-meta">Total cabin bookings</div>
          </div>
              ...
              <table className="my-revenue__table">
                ...
                <tbody>
                  {displayBookings.length === 0 ? (
                    <tr><td colSpan={8}><div className="my-revenue__empty"><Receipt size={32} className="my-revenue__empty-icon" /><p className="my-revenue__empty-text">No cabin bookings found</p><p className="text-xs text-gray-400">Only cabin bookings are shown here</p></div></td></tr>
                  ) : (
                    displayBookings.map((b, idx) => {
                      ...
                      return (
                        <tr key={b._id}>
                          ...
                          <td>
                            <div><p className="font-semibold text-gray-900 text-xs flex items-center gap-1">{b.cabin?.name || 'Unknown'}<span className="text-[8px] bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-1.5 py-0.5 rounded-full font-medium">Cabin</span></p><p className="text-[9px] text-gray-400">{b.cabin?.address?.split(',')[0] || 'N/A'}</p></div>
                          </td>
