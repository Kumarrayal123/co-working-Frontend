// MyCafePayments.jsx - Complete with Multi-Filters (Cafe Name, Date Range, Status)
...
const MyCafePayments = () => {
  ...
  const [filterCafeName, setFilterCafeName] = useState("");
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      // ✅ Filter only cafe bookings (isCafe === true)
      const allOrders = res.data.orders || [];
      const cafeOrders = allOrders.filter(order => order.cabin?.isCafe === true);

      setOrders(cafeOrders);
      setStats(res.data.stats || { total: 0, active: 0, expired: 0, totalAmount: 0 });

    } catch (err) {
      console.error("Error fetching cafe payments:", err);
      toast.error("Failed to fetch cafe payment history");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredOrders = orders.filter(order => {
    ...
    // Cafe name filter
    const matchCafeName = filterCafeName === "" || 
      order.cabin?.name?.toLowerCase().includes(filterCafeName.toLowerCase());
    ...
    return matchStatus && matchCafeName && matchDate;
  });
  ...
  const getCafeName = (order) => {
    return order.cabin?.name || 'Cafe Deleted';
  };

  const getCafeAddress = (order) => {
  ...
  const exportToExcel = () => {
    try {
      ...
      const exportData = sortedOrders.map((order, index) => ({
        'S.No': index + 1,
        'Cafe Name': getCafeName(order),
        'Address': getCafeAddress(order),
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Cafe_Payments');

      const date = new Date().toISOString().split('T')[0];
      XLSX.writeFile(wb, `cafe_payments_${date}.xlsx`);
  ...
  const downloadInvoice = (order) => {
    try {
      ...
      const cafeName = cabin.name || 'Cafe Deleted';
      const cafeAddress = cabin.address || 'N/A';
          ...
          <body>
            <div class="invoice-container">
              <div class="invoice-header">
                <div class="brand"><h1>${cafeName.toUpperCase()}</h1><div class="gst">GST: ${(gstRate * 100).toFixed(0)}%</div><div class="address-line">${cafeAddress}</div></div>
                <div class="invoice-number"><div class="label">Invoice</div><div class="number">#${orderId}</div><div class="date">${today}</div></div>
              </div>
              <div class="info-grid">
                <div><div class="title">Bill To</div><div class="value">${localStorage.getItem('userName') || 'Customer'}</div><div class="value-small">Cafe Registration</div></div>
                <div><div class="title">Cafe Details</div><div class="value">${cafeName}</div><div class="address-line">${cafeAddress}</div><div class="value-small" style="margin-top:4px;">${order.isFirstCabin ? '⭐ First Cafe' : 'Payment #' + paymentCount}</div></div>
              </div>
              <table class="invoice-table"><thead><tr><th>Description</th><th>Details</th><th>Amount</th></tr></thead>
                <tbody><tr><td><strong>Cafe Registration</strong></td><td>${cafeName}<br><span style="font-size:11px;color:#666666;">${order.isFirstCabin ? 'First Cafe Registration' : 'Cafe Registration'}</span>${transactionId !== 'N/A' ? `<br><span style="font-size:10px;color:#888888;font-family:monospace;">TXN: ${transactionId}</span>` : ''}${paymentCount > 1 ? `<br><span style="font-size:11px;color:#059669;font-weight:600;">🔄 Renewal #${paymentCount}</span>` : ''}</td><td>₹${baseAmount.toFixed(2)}</td></tr></tbody></table>
              ...
              <div class="footer"><div class="powered">POWERED BY <span>IRYAX SPACE</span></div><div class="sub">Thank you for choosing ${cafeName}</div><div class="sub" style="margin-top:2px;">This is a system generated invoice</div></div>
            </div>
      ...
      <div className="pt-24 px-3 sm:px-4 md:px-6 lg:px-8 max-w-full mx-auto pb-16">
        ...
        <div className="admin-dash__header">
          <div>
            <h1 className="admin-dash__greeting" style={{ fontSize: '1.25rem' }}>
              My <span style={{ color: '#4f46e5' }}>Cafe Billings</span>
            </h1>
            <p className="admin-dash__subtitle" style={{ fontSize: '11px' }}>
              Manage your cafe payment history
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm">
            ...
            <div className="text-[9px] text-gray-400">Currently active cafes</div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm">
            ...
            <div className="text-[9px] text-gray-400">Lifetime cafe payments</div>
          </div>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={filterCafeName}
                  onChange={(e) => setFilterCafeName(e.target.value)}
          ...
          <div className="admin-dash__card-body p-0 overflow-x-auto" style={{ backgroundColor: '#ffffff' }}>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center gap-2 py-16 text-gray-400">
                ...
                <p className="text-base font-medium">No cafe payments found</p>
                <p className="text-xs">Add a cafe to start your payment history.</p>
              </div>
            ) : (
                      ...
                      <tr key={order._id} className="transition-colors group hover:bg-gray-50/80">
                        ...
                        <td className="px-3 py-2.5">
                          <div>
                            ...
                            <p className="text-[9px] text-gray-500 flex items-center gap-0.5 mt-0.5">
                              ...
                              {getCafeAddress(order)}
                            </p>
                            {order.cabin === null && (
                              <span className="text-[8px] text-red-500 font-medium">⚠️ Cafe Deleted</span>
                            )}
                          </div>
                        </td>
                        <td className="px-3 py-2.5">
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {/* ✅ ONLY VIEW BILLING AND DOWNLOAD INVOICE */}
                            <button
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Cafe Details</p>
                <p className="mt-1 font-semibold text-gray-800 text-sm">{getCafeName(selectedOrder)}</p>
                <p className="text-xs text-gray-600 flex items-center gap-0.5 mt-0.5">
                  <MapPin size={12} /> {getCafeAddress(selectedOrder)}
                </p>
                {selectedOrder.isFirstCabin && (
                  <span className="text-[10px] text-indigo-600 font-medium mt-0.5 inline-block">⭐ First Cafe</span>
                )}
              </div>
