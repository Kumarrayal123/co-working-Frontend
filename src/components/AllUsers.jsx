...
import axios from "axios";
import { CheckCircle, Clock, Download, Eye, File, FileText, Search, Trash2, User, XCircle } from "lucide-react";
...
function AllUsers() {
  ...
  // ✅ ADD THIS FUNCTION (component ke andar)
const updateSingleDocumentStatus = (docKey, status) => {
  if (!selectedUser) return;

  const payload = {
    adharCardStatus: selectedUser.adharCardStatus || "pending",
    panCardStatus: selectedUser.panCardStatus || "pending",
    mbbsCertificateStatus: selectedUser.mbbsCertificateStatus || "pending",
    pmcRegistrationStatus: selectedUser.pmcRegistrationStatus || "pending",
    nmrIdStatus: selectedUser.nmrIdStatus || "pending",
    status: selectedUser.status || "active",
    [`${docKey}Status`]: status
  };
          ...
          {documentKeys.map((key) => {
            ...
            return (
              <div key={key} className="flex items-center justify-between p-3 bg-white rounded-lg border">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 text-xs rounded ${getStatusColor(documentStatus)}`}>
                    {documentStatus}
                  </span>
                  ...
                  <button onClick={() => updateSingleDocumentStatus(key, "approved")}>
                    <CheckCircle size={16} className="text-green-600" />
                  </button>

                  <button onClick={() => updateSingleDocumentStatus(key, "rejected")}>
