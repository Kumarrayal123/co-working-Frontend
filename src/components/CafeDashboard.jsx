// CafeDashboard.jsx - With Profile Completion Circle - Cabin Bookings Chart with Hover Tooltip - FILTERS ONLY ON CHART with Custom Date Range (Blue Theme)
...
const CafeDashboard = () => {
  ...
  const calculateCompletion = (userData) => {
    const fields = [
      ...
      { key: 'panCardStatus', label: 'PAN Card', required: false },
    ];
  ...
  const applyFilters = () => {
    ...
    toast.success(`Filtered: ${filtered.length} cafe bookings found`);
  };
  ...
  // Stats Cards - Cafe specific with Blue Theme
  const statsCards = [
    {
      label: "My Bookings",
      value: totalBookings,
  ...
  const profileName = profile?.name || user?.name || 'Cafe Owner';
  ...
  return (
    <div className="admin-dash" style={{ backgroundColor: '#ffffff' }}>
      ...
      <div className="pt-20 px-3 sm:px-4 md:px-6 lg:px-8 max-w-full mx-auto pb-16">
        ...
        {/* Profile Completion Card - Blue Theme */}
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                <div className="flex items-center gap-1 bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-lg border border-gray-200">
                  ...
                  <span className="text-xs font-bold text-indigo-600">{missingFields.length}</span>
                </div>
              </div>

              {missingFields.length > 0 && (
                <div className="mt-1 flex flex-wrap items-center gap-1 bg-white/60 backdrop-blur-sm px-2 py-1 rounded-lg border border-indigo-200">
                  <AlertTriangle size={10} className="text-indigo-500 flex-shrink-0" />
                  <p className="text-[8px] text-gray-700">
                    <span className="font-semibold text-indigo-600">{missingFields.length}</span> fields remaining
                  </p>
                  <button
                    onClick={() => navigate("/cafeprofile")}
        ...
        {/* Filter Section - Only for Chart - Blue Theme */}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400">Filter cafe bookings chart</p>
              </div>
            </div>
        ...
        {/* Cabin Bookings Chart - Full Width with Hover Tooltip - Blue Theme */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800">Cafe Bookings</h3>
            ...
            {cabinChartData.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[7px] font-bold ${item.bookings > 0 ? 'text-indigo-600' : 'text-gray-300'}`}>
                    {item.bookings}
                  </span>
                </div>
              );
          ...
          {hoveredBar && (
            ...
            >
              <div className="font-bold text-indigo-400 text-sm">{hoveredBar.item.month}</div>
          ...
          <div className="flex justify-between mt-3 text-[10px] text-gray-500 bg-gray-50 rounded-xl px-4 py-2">
            <span className="flex items-center gap-2 font-medium">
              ...
              Total: {cabinChartData.reduce((sum, d) => sum + d.bookings, 0) || 0} cafe bookings
            </span>
            <span className="flex items-center gap-2 font-medium">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
          <div className="flex flex-wrap items-center justify-between gap-3 p-5 border-b border-gray-100 bg-gray-50/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800">My Cafes</h3>
                <p className="text-xs text-gray-500">Manage your registered cafes</p>
              </div>
            ...
            {cabins.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 py-16 text-gray-400">
                ...
                <div className="text-center">
                  <p className="text-sm font-semibold text-gray-500">No cafes found</p>
                  <p className="text-xs text-gray-400 mt-1">You haven't registered any cafes yet.</p>
                </div>
              </div>
            ) : (
              <table className="w-full min-w-[700px] text-left">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th className="p-4 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Cafe</th>
                    <th className="p-4 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Address</th>
                    <th className="p-4 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Cuisine</th>
                  ...
                  {cabins.slice(0, 5).map((cabin, index) => {
                    ...
                    return (
                      <tr key={cabin._id} className="transition-all hover:bg-gradient-to-r hover:from-gray-50 hover:to-indigo-50/30 cursor-pointer group" onClick={() => navigate(`/cabin/${cabin._id}`)}>
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-gray-500 font-medium">{cabin.cuisineType || 'N/A'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold rounded-lg bg-indigo-100 text-indigo-700 border border-indigo-200">
                            <UtensilsCrossed size={11} />
                            {cabin.cuisineType || 'Cafe'}
                          </span>
                        </td>
                        <td className="p-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5 border-b border-gray-100 bg-gray-50/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Recent cafe reservations</p>
              </div>
              <span className="px-3 py-1 text-xs font-bold text-indigo-700 bg-indigo-100 rounded-full">
              ...
              <table className="w-full min-w-[700px] text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {latestMyBookings.map((b, idx) => {
                    ...
                    return (
                      <tr key={b._id} className="transition-all hover:bg-gradient-to-r hover:from-gray-50 hover:to-indigo-50/30 cursor-pointer group" onClick={() => navigate("/cafebookings")}>
                        <td className="p-4">
                          <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-xs font-bold text-gray-500 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors">
                            {idx + 1}
                          </span>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
          <div className="flex flex-wrap items-center justify-between gap-3 p-5 border-b border-gray-100 bg-gray-50/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800">My Cafe Bookings</h3>
                <p className="text-xs text-gray-500">Bookings received for your cafes</p>
              </div>
          ...
          <div className="p-0 overflow-x-auto">
            {latestCabinBookings.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 py-16 text-gray-400">
                ...
                <div className="text-center">
                  <p className="text-sm font-semibold text-gray-500">No cafe bookings found</p>
                  <p className="text-xs text-gray-400 mt-1">No one has booked your cafes yet.</p>
                </div>
              </div>
            ) : (
              <table className="w-full min-w-[700px] text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {latestCabinBookings.map((b, idx) => {
                    ...
                    return (
                      <tr key={b._id} className="transition-all hover:bg-gradient-to-r hover:from-gray-50 hover:to-rose-50/30 cursor-pointer group" onClick={() => navigate("/cafebookings")}>
                        <td className="p-4">
