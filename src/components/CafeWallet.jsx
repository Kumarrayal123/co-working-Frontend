// CafeWallet.jsx - Complete Cafe Wallet (Same as DoctorWallet)
...
const CafeWallet = () => {
  ...
  const fetchWallet = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Remove duplicates by using a Map with _id as key
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Cafe_Wallet');

      const date = new Date().toISOString().split('T')[0];
      XLSX.writeFile(wb, `cafe_wallet_${date}.xlsx`);
        ...
        <div className="admin-dash__header">
          <div>
            <h1 className="admin-dash__greeting" style={{ fontSize: '1.25rem' }}>
              <span style={{ color: '#4f46e5' }}>Cafe</span> <span>Wallet</span>
            </h1>
            <p className="admin-dash__subtitle" style={{ fontSize: '11px' }}>
              Manage your cafe earnings and withdrawals
            </p>
          </div>
        </div>
                          ...
                          <td className="px-3 py-2.5">
                            <div>
                              <p className="font-semibold text-gray-900 text-xs">
                                {transaction.cabinName || "Unknown Cafe"}
                              </p>
