...
//           'Booking Type': booking.bookingType || 'booking',
...
//           'Start Date': formatDateIndian(booking.startDate),
//           'Start Time': formatTime12(booking.startTime),
//           'End Date': formatDateIndian(booking.endDate),
...
//           'Payment Mode': booking.paymentDetails?.mode || 'N/A',
...
//           'Is Extended': booking.isExtended ? 'Yes' : 'No',
//           'Is Replaced': booking.isReplaced ? 'Yes' : 'No',
//           'Created At': formatDateTimeIndian(booking.createdAt)
//         };
//       });
...
//         `<span style="display:inline-block;background:#eff6ff;padding:1px 8px;border-radius:10px;margin:1px;font-size:8px;border:1px solid #93c5fd;">${formatDateFn(s.date)} ${s.startTime}-${s.endTime} (${s.hours}h)</span>`
//       ).join('');
//     }
...
//         ${totalDays > 0 ? `<div style="display:flex;justify-content:space-between;padding:1px 0;font-size:10px;"><span style="color:#555;">Days</span><span style="font-weight:600;">${totalDays}</span></div>` : ''}
...
//             <div style="font-size:8px;color:#555;margin-bottom:2px;">Slots (${booking.bookingSlots.length} days):</div>
//             <div style="display:flex;flex-wrap:wrap;gap:2px;">${slotsHtml}</div>
//             <div style="font-size:8px;color:#888;margin-top:2px;">Daily: ${booking.dailyHours?.join(', ') || 'N/A'}h</div>
//           </div>
//         ` : ''}
...
//                     {activeTab === 'visits' ? (
//                       <>
//                         <th className="p-2 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Visit Date</th>
//                         <th className="p-2 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Visit Time</th>
...
//                         <th className="p-2 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Days</th>
//                         <th className="p-2 text-[10px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Daily Hrs</th>
...
//                     const totalDays = booking.totalDays || 0;
//                     const dailyHours = booking.dailyHours?.join(', ') || 'N/A';
...
//                             </td>
//                             <td className="p-2">
//                               <span className="text-xs font-medium text-gray-700">{formatTime12(booking.startTime)}</span>
//                             </td>
//                             <td className="p-2">
...
//                             </td>
//                             <td className="p-2">
//                               <span className="text-[10px] text-gray-500 font-medium">{formatDateTimeIndian(booking.createdAt)}</span>
//                             </td>
...
//                             </td>
//                             <td className="p-2">
//                               <span className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full text-[10px] font-bold">{totalDays}d</span>
//                             </td>
//                             <td className="p-2">
...
//                     <p><span className="text-gray-500">Capacity:</span> <span className="font-medium">{viewBooking.cabin?.capacity || 'N/A'}</span></p>
//                     <p><span className="text-gray-500">Price:</span> <span className="font-medium">₹{viewBooking.cabin?.price || 0}/hr</span></p>
//                   </div>
//                 </div>
...
//                     <p className="font-semibold">{viewBooking.totalDays || 0} days</p>
//                   </div>
//                   <div>
...
//                     <p className="font-semibold">{viewBooking.dailyHours?.join(', ') || 'N/A'}</p>
//                   </div>
//                 </div>
//               </div>
...
//                 <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
//                   <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider flex items-center gap-2">
//                     <CalendarDays size={14} /> Booking Slots ({viewBooking.bookingSlots.length} days)
//                   </p>
...
const AllBookings = () => {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const matchesSearch = !filterSearch ||
      b.cabin?.name?.toLowerCase().includes(search) ||
      b.cabin?.address?.toLowerCase().includes(search) ||
      b.name?.toLowerCase().includes(search) ||
      b.mobile?.includes(filterSearch) ||
    ...
    return matchesDateFrom && matchesDateTo && matchesCabinName && matchesOwner && 
           matchesStatus && matchesPaymentStatus && matchesPaymentMethod && matchesSpaceType && matchesSearch;
  });
  ...
  return (
    <div className="admin-dash" style={{ backgroundColor: '#ffffff' }}>
      ...
      <div className="pt-20 px-3 sm:px-4 md:px-6 lg:px-8 max-w-full mx-auto pb-16">
        ...
        {/* Stats Cards - Exact MyBookings style */}
          ...
          <div className="overflow-x-auto">
            {displayBookings.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 py-20 text-gray-400">
                ...
                {hasActiveFilters && (
                  <button onClick={clearFilters} className="mt-2 px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-medium hover:bg-indigo-100 transition">
                    Clear Filters
                  </button>
                )}
              </div>
              ...
              <table className="w-full min-w-[1100px] text-left text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">S.No</th>
                    {/* <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Booking ID</th> */}
                    <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Cabin</th>
                    <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap"> Space Type</th>
                    <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Customer</th>
                    {activeTab === 'visits' ? (
                      <>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Visit Date</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Visit Time</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Status</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Created At</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap text-center">Actions</th>
                      </>
                    ) : (
                      <>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Start</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">End</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Hours</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Days</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Seats</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Status</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Payment</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Pmt Status</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Visits</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">Amount</th>
                        <th className="px-3 py-2 text-[9px] font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap text-center">Actions</th>
                      </>
                    )}
                  </tr>
                  ...
                  {displayBookings.map((booking, idx) => {
                    ...
                    return (
                      <tr key={booking._id} className="hover:bg-gray-50/80 transition-colors border-b border-gray-50 last:border-0">
                        <td className="px-3 py-2"><span className="text-[10px] font-semibold text-gray-400">{idx + 1}</span></td>
                        {/* <td className="px-3 py-2">
                          <div>
                            <p className="font-mono text-[10px] font-bold text-indigo-600">{bookingId}</p>
                            <p className="text-[9px] text-gray-400">{isVisit ? '🗓 Visit' : '📦 Booking'}</p>
                          </div>
                        </td> */}
                        <td className="px-3 py-2">
                          <div>
                            <p className="font-semibold text-gray-900 text-xs">{booking.cabin?.name || "Unknown"}</p>
                        ...
                        <td className="px-3 py-2">
                          <div>
                            <p className="font-semibold text-gray-900 text-xs">{booking.name || booking.user?.name || "Unknown"}</p>
                        ...
                        {isVisit ? (
                          ...
                          <>
                            <td className="px-3 py-2">
                              <div>
                                <span className="text-xs font-medium text-gray-700">{formatDateIndian(booking.startDate)}</span>
                                <p className="text-[9px] text-indigo-600 font-medium">{formatTime12(booking.startTime)}</p>
                              </div>
                            </td>
                            <td className="px-3 py-2">
                              <span className="text-xs font-medium text-gray-700">{formatTime12(booking.startTime)}</span>
                            ...
                            <td className="px-3 py-2">
                              <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full ${statusBadge.color}`}>{statusBadge.label}</span>
                            </td>
                            <td className="px-3 py-2">
                              <span className="text-[10px] text-gray-500 font-medium">{formatDateTimeIndian(booking.createdAt)}</span>
                            </td>
                            <td className="px-3 py-2 text-center">
                              <div className="flex items-center justify-center gap-1">
                                <button 
                                  onClick={() => handleViewBooking(booking)} 
                                  className="p-1.5 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 transition" title="View"
                                >
                                  <Eye size={13} />
                                </button>
                                <button 
                                  onClick={() => { setSelectedBooking(booking); setNewStatus(booking.status || 'pending'); setShowStatusModal(true); }} 
                                  className="p-1.5 bg-orange-50 text-orange-700 rounded-lg hover:bg-orange-100 transition" title="Update Status"
                                >
                                  <Edit size={13} />
                                </button>
                                <button 
                                  onClick={() => { setDeleteBooking(booking); setShowDeleteModal(true); }} 
                                  className="p-1.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition" title="Delete"
                                >
                                  <Trash2 size={13} />
                                </button>
                              </div>
                          ...
                          <>
                            ...
                            <td className="px-3 py-2">
                              <div>
                                <span className="text-xs font-medium text-gray-700">{formatDateIndian(booking.endDate)}</span>
                                <p className="text-[9px] text-indigo-600 font-medium">{formatTime12(booking.endTime)}</p>
                              </div>
                            </td>
                            <td className="px-3 py-2">
                              <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded-full text-[9px] font-bold">{booking.totalHours}h</span>
                            </td>
                            <td className="px-3 py-2">
                            ...
                            <td className="px-3 py-2">
                              <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full ${paymentMethodBadge.color}`}>{paymentMethodBadge.label}</span>
                            </td>
                            <td className="px-3 py-2">
                              <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full ${paymentStatusBadge.color}`}>{paymentStatusBadge.label}</span>
                            </td>
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                <History size={12} className="text-indigo-400" />
                            ...
                            <td className="px-3 py-2">
                              <span className="text-xs font-bold text-indigo-600">₹{booking.totalPrice?.toLocaleString("en-IN") || "0"}</span>
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center justify-center gap-1 flex-wrap">
                                ...
                                <button 
                                  onClick={() => printReceipt(booking)} 
                                  className="p-1.5 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition" title="Print Receipt"
                                >
                                  <Printer size={13} />
                                </button>
                                <button 
                                  onClick={() => downloadPDF(booking)} 
                                  className="p-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition" title="Download PDF"
                                >
                                  <FileDown size={13} />
                                </button>
                                <button 
                                  onClick={() => { setTimingBooking(booking); setNewTiming({ date: "", checkIn: "", checkOut: "" }); setShowTimingModal(true); }} 
                                  className="p-1.5 bg-cyan-50 text-cyan-700 rounded-lg hover:bg-cyan-100 transition" title="Add Timing"
                                >
                                  <Plus size={13} />
                                </button>
                                <button 
                                ...
                                <button 
                                  onClick={() => {
                                    ...
                                    setNewPaymentStatus(booking.paymentStatus || 'pending');
                                    setAmountPaid(booking.totalPrice || 0);
                                    setPaymentDetails({ paymentMode: 'cash', cardNumber: '', cardHolderName: '', cardExpiry: '', cardCVV: '', upiId: '', upiApp: '', transactionId: '', paymentDate: new Date().toISOString().split('T')[0], notes: '' });
                                    setPaymentScreenshot(null);
                                    setPaymentScreenshotPreview(null);
                                    setShowPaymentModal(true);
                                  }} 
                                  className="p-1.5 bg-yellow-50 text-yellow-700 rounded-lg hover:bg-yellow-100 transition" title="Update Payment"
                                >
                                  <CreditCard size={13} />
                                </button>
                                <button 
