// AdminProfile.jsx - Admin Profile Page (No Completion Percentage)
    ...
    if (!adminId) {
      ...
      if (adminString) {
        try {
          ...
          adminId = adminData._id || adminData.id || adminData.userId;
          if (adminId) {
    ...
    if (!adminId) {
      try {
        ...
        if (token) {
          ...
          adminId = payload.id || payload.userId || payload._id || payload.sub;
          if (adminId) {
            localStorage.setItem("userId", adminId);
          }
        }
  ...
  const getLoginInfo = () => {
    ...
    if (adminString) {
      try {
        const adminData = JSON.parse(adminString);
        setLoginInfo({
          loginName: adminData.name || adminData.email || "Admin",
          loginEmail: adminData.email || "admin@iryax.com",
      ...
      if (userString) {
        try {
          ...
          setLoginInfo({
            loginName: userData.name || userData.email || "Admin",
            loginEmail: userData.email || "admin@iryax.com",
            loginTime: new Date().toLocaleString('en-IN', {
              day: '2-digit',
              month: 'short',
              year: 'numeric',
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/auth/profile/${adminId}`,
        getAuthHeader()
      );
      ...
      if (res.data.success && res.data.user) {
        ...
        console.error("❌ Profile API error:", res.data);
      }
    } catch (err) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-800">{loginInfo.loginName || profile.name || "Admin"}</p>
                <p className="text-xs text-gray-500">{loginInfo.loginEmail || profile.email || "admin@iryax.com"}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-400">Login Time</p>
              <p className="text-sm font-medium text-gray-700">{loginInfo.loginTime || "Just now"}</p>
            </div>
          </div>
        </div>
